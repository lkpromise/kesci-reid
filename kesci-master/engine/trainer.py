...
def create_supervised_trainer(model, optimizer, loss_fn,
    ...
    def _update(engine, batch):
        ...
        loss,cross,tri = loss_fn(score, feat, target) ## modified by liu
        loss.backward()
        optimizer.step()
        # compute acc
...
def create_supervised_trainer_with_center(model, center_criterion, optimizer, optimizer_center, loss_fn, cetner_loss_weight,
    ...
    def _update(engine, batch):
        ...
        target = target.to(device) if torch.cuda.device_count() >= 1 else target
        score, feat = model(img)
        loss,cross,tri,center = loss_fn(score, feat, target)  ## modified by liu
        # print("Total loss is {}, center loss is {}".format(loss, center_criterion(feat, target)))
        loss.backward()
        optimizer.step()
        ## 如果改回之前的存储方式需要，把这里再去除注释，这里应该是把center的权重再乘上梯度上
        for param in center_criterion.parameters():
            #print(param)
            # if(param.grad==None):
            #     print("当前是第一次传播，所以一开始为空")
            #     continue
...
def do_train(
    ...
    RunningAverage(output_transform=lambda x: x[2]).attach(trainer, 'cross_loss')
    RunningAverage(output_transform=lambda x: x[3]).attach(trainer, 'tri_loss')
    ...
    def log_training_loss(engine):
        ...
        if ITER % log_period == 0:
            logger.info("Epoch[{}] Iteration[{}/{}] Loss: {:.3f},cross_loss:{:.3f},tri_loss:{:.3f}, Acc: {:.3f}, Base Lr: {:.2e}"
                        .format(engine.state.epoch, ITER, len(train_loader),
                                engine.state.metrics['avg_loss'],
                                engine.state.metrics['cross_loss'],
...
def do_train_with_center(
    ...
    RunningAverage(output_transform=lambda x: x[2]).attach(trainer, 'cross_loss')
    RunningAverage(output_transform=lambda x: x[3]).attach(trainer, 'tri_loss')
    RunningAverage(output_transform=lambda x: x[4]).attach(trainer, 'center_loss') ## modified by liu
